...
try:
    ...
    if has_changes:
        ...
        body += f'<h2 style="color:#1e3a5f;border-bottom:2px solid #3b82f6;padding-bottom:6px">Soltri 선생산 변경 알림 ({TODAY.strftime("%Y-%m-%d %H:%M")})</h2>'

        if stock_alert:
            body += f'<h3 style="color:#f59e0b">⚠️ 재고 소진 경고 ({len(stock_alert)}개)</h3>'
            body += '<p style="font-size:11px;color:#666">재고율이 10% 이하로 떨어진 품목</p>'
            body += '<table border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;font-size:12px">'
            body += '<tr style="background:#fff8e1"><th>치수</th><th>재질</th><th>현재고</th><th>재고율</th><th>주문횟수</th><th>총량</th></tr>'
            for r in stock_alert:
                body += f'<tr><td>{r["chisu"]}</td><td>{r["jaejil"]}</td><td>{r["stock"]}개</td><td style="color:#ef4444;font-weight:bold">{r["stock_ratio"]}%</td><td>{r["freq"]}회</td><td>{r["total_qty"]}</td></tr>'
            body += '</table><br>'

        if new_keys:
            body += f'<h3 style="color:#22c55e">🟢 신규 품목 ({len(new_keys)}개)</h3>'
            body += '<p style="font-size:11px;color:#666">새로 선생산 대상에 추가된 품목</p>'
            body += '<table border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;font-size:12px">'
            body += '<tr style="background:#e8f5e9"><th>치수</th><th>재질</th><th>주문횟수</th><th>총량</th><th>현재고</th><th>재고율</th></tr>'
            for k in new_keys:
                r = curr_items[k]
                body += f'<tr><td>{r["chisu"]}</td><td>{r["jaejil"]}</td><td>{r["freq"]}회</td><td>{r["total_qty"]}</td><td>{r["stock"]}</td><td>{r["stock_ratio"]}%</td></tr>'
            body += '</table><br>'

        if removed_keys:
            body += f'<h3 style="color:#94a3b8">제거된 품목 ({len(removed_keys)}개)</h3>'
            body += '<p style="font-size:11px;color:#666">조건 미달로 목록에서 빠진 품목</p>'
            body += '<table border="1" cellpadding="5" cellspacing="0" style="border-collapse:collapse;font-size:12px">'
            body += '<tr style="background:#f1f5f9"><th>치수</th><th>재질</th></tr>'
            for k in removed_keys:
                parts = k.split('|')
                body += f'<tr><td>{parts[0]}</td><td>{parts[1] if len(parts)>1 else ""}</td></tr>'
            body += '</table><br>'

        body += f'<p style="margin-top:16px"><a href="https://nuckythompson28-code.github.io/frequency/안전재고발주.html" style="background:#3b82f6;color:#fff;padding:8px 16px;border-radius:6px;text-decoration:none;font-weight:bold">대시보드 열기</a></p>'
        body += '</body></html>'

        # 제목
        changes = []
        if stock_alert: changes.append(f'⚠️재고소진 {len(stock_alert)}')
        if new_keys: changes.append(f'신규 {len(new_keys)}')
        if removed_keys: changes.append(f'제거 {len(removed_keys)}')
        subject = f'[선생산] {", ".join(changes)}'
